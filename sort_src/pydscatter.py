...
def dscatter_plot(X, Y, nbins=[], order=False, lamb=20, log=[False, False], markersize=5, ax=None, **kwargs):
    ...
    if log == [False, False]:
        ...
        col, _, _, _ = pydscatter(X, np.log10(Y), nbins, lamb)
    elif log == [True, False]:
        col, _, _, _ = pydscatter(np.log10(X), Y, nbins, lamb)
    elif log == [True, True]:
        col, _, _, _ = pydscatter(np.log10(X), np.log10(Y), nbins, lamb)
...
def dscatter_contour(X, Y, nbins=[], lamb=20, log=[False, False], ax=None, **kwargs):
    ...
    if log == [False, False]:
        _, F, cntr1, cntr2 = pydscatter(X, Y, nbins, lamb)
    elif log == [False, True]:
        _, F, cntr1, cntr2 = pydscatter(X, np.log10(Y), nbins, lamb)
    elif log == [True, False]:
        _, F, cntr1, cntr2 = pydscatter(np.log10(X), Y, nbins, lamb)
    elif log == [True, True]:
        _, F, cntr1, cntr2 = pydscatter(np.log10(X), np.log10(Y), nbins, lamb)
...
def aggregate(group_idx, a, size):
    ...
    ret = np.bincount(group_idx, minlength=np.prod(size))
    if a != 1:
        ret *= a

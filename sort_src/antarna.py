...
##############################
# This is directly adapted from "antaRNA: ant colony-based RNA sequence design"
# https://doi.org/10.1093/bioinformatics/btv319
# The code below was translated from Python 2 into Python 3, and slightly modified as needed.
##############################
...
def applyTerrainModification(args):
    ...
    if args.modus == "MFE": 
        ...
        for terrainelement in args.terrain:
            ...
            # print(id1.type ,len(args.Cseq), len(successors))
            if float(id1) < len(args.Cseq) and len(successors) == 0:
...
class Variables:
    ...
    def parse_GC_management(self):
        ...
        if type(self.tGC) is float: # CASE Only one tGC value is defined, which needs to account for the whole terrain
            self.tGC = [(self.tGC, 0, self.length - 1)]
    ...
    def checkForViennaTools(self):
        ...
        """
        RNAfold_output = str(subprocess.Popen(["which", "RNAfold"], stdout=subprocess.PIPE).communicate()[0].strip())
